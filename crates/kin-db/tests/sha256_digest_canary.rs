...
//! Backend-independence canary for the SHA-256 behind stored-body identity.
//!
...
//! two different code paths on two different machines. Everything this crate
//! addresses by content depends on that being invisible — body checksums,
//! snapshot digests, Merkle roots, admission records — and a store written on
//! one host has to keep validating on the other, since `validate_all_authority_bodies`
//! re-derives those digests on every open and refuses the store when they differ.
//!
...
fn streaming_updates_match_one_shot_across_unaligned_chunks() {
    ...
    // get wrong, and the way this crate actually hashes, field by field.
