...
impl CrossEncoder {
    pub fn new(model_id: &str, revision: &str) -> Result<Self, KinDbError> {
        ...
        let model = BertModel::load(&weights_path, config).map_err(|e| {
            KinDbError::IndexError(format!("failed to load cross-encoder model: {e}"))
        })?;
    ...
    pub fn rerank(&self, query: &str, documents: &[&str]) -> Result<Vec<f32>, KinDbError> {
        ...
        self.model
            .forward_cross_encoder_batched(&token_ids, &attention_masks)
