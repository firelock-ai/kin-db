...
fn load_1d(tensors: &SafeTensors, name: &str, expected: usize) -> Result<Array1<f32>, KinDbError> {
    let view = tensors
        .tensor(name)
...
fn dequantize_q8_block(block: &[u8]) -> [f32; 32] {
...
fn dequantize_q4_block(block: &[u8]) -> [f32; 32] {
...
fn try_load_2d_flexible(
    ...
    rows: usize,
    cols: usize,
) -> Result<Option<Array2<f32>>, KinDbError> {
    match resolve_name(tensors, &candidates(suffixes)) {
...
fn load_safetensors_bytes(weights_path: &Path) -> Result<Vec<u8>, KinDbError> {
...
fn load_sharded_index(index_path: &Path) -> Result<HashMap<String, String>, KinDbError> {
    let data = std::fs::read_to_string(index_path)
        .map_err(|e| KinDbError::IndexError(format!("failed to read shard index: {e}")))?;
    let parsed: serde_json::Value = serde_json::from_str(&data)
        .map_err(|e| KinDbError::IndexError(format!("failed to parse shard index: {e}")))?;
    let weight_map = parsed
        .get("weight_map")
        .and_then(|v| v.as_object())
        .ok_or_else(|| KinDbError::IndexError("shard index missing weight_map".into()))?;
...
impl BertModel {
    ...
    fn load_from_tensors(tensors: &SafeTensors, config: BertConfig) -> Result<Self, KinDbError> {
...
/// RMSNorm: x * rsqrt(mean(x²) + eps) * weight. No bias, no mean subtraction.
