...
impl GraphSnapshot {
    ...
    /// one caller entitled to state it is `prepare_successor`, which computed
    /// the bundle it is asking this gate not to compute again. Every other
    /// entry point here recomputes, so a snapshot read from disk is still
    /// checked against its own envelope.
    ///
