# Contributing to kin-db

Thanks for your interest in kin-db. This guide covers local development, the
...
```sh
git commit -s -m "fix(storage): correct snapshot swap under concurrent reads"
```
...
feat(vector): add HNSW persistence for graph-restart warmup
perf(retrieve): parallelize BM25 + vector arms with rayon::join
```

Common types are `feat`, `fix`, `docs`, `test`, `refactor`, `perf`, and
`chore`. Scopes match the area you touched (`storage`, `vector`, `search`,
`snapshot`, `embed`, and so on). Write the summary in the imperative mood and
...
kin-db is one repository within a larger ecosystem. The system of record CLI,
daemon, MCP server, projections, and reconcile logic live in `kin`. Embedding
inference lives in `kin-infer`. If your change targets one of those concerns,
open it against the repository that owns the code.
